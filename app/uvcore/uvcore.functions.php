...
include_once("libs/uvcore.lib.php");
include_once("libs/feedsinfo.lib.php");
include_once("libs/forms.lib.php");
include_once("libs/designtemplates.lib.php");
include_once("libs/program.lib.php");
include_once("libs/uvmap.lib.php");
...
function uv_get_media_feed($uvmediaid, $uvmediaelement){
	...
	else if(!$uvmediatypeid and $uvfeedcotent["imagegallery"])
		$uvmediatypeid = $uvfeedcotent["imagegallery"][0]["mediatypeid"];
	...
	if($uv_feedspath and (preg_match("/^.+\.(\w{3,4})$/", $uvfeedurl, $uvfeedurlparts) or preg_match("/^.+\.(\w{3,4})[\?].+$/", $uvfeedurl, $uvfeedurlparts))){
		...
		if(file_exists("$uvfeedfullpath")){
			...
			if($uvnowtime > $uvfileexpiresat){
				...
				$uvfilecontent = file_get_contents($uvfeedfullpath);
...
function uv_get_eventlisthtml($uveventlisttemplate, $uvineventtemplate = "default", $uvargs = false){
...
function uv_get_images_gallery_list($uvmediaid, $uvlisttemplate){
	$uvimgalfeed = uv_get_feed($uvmediaid);
	
	if(is_array($uvimgalfeed))
		foreach($uvimgalfeed as $uvimgal){
			$uvimgalfolder = $uvimgal["imagefolder"];
			$uvimgalfile = $uvimgal["imagefile"];
			
			$uvimglist .= str_replace(array("{imagefolder}", "{imagefile}"), array($uvimgalfolder, $uvimgalfile), $uvlisttemplate);
		}
